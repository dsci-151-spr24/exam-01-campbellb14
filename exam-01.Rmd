...
```
**Answer: The top ten most common destinations for flights from NYC airports in 2013 are ORD, ATL, LAX, BOS, MCO, CLT, SFO, FLL, MIA, and DCA. ORD being the most common and DCA being the least common.** **Approach: First, I grouped by the destination variable. Then, I needed the count of flights to each destination. Next, I sorted the results in descending order. Finally, I put only the top 10 in the table. Lastly, I used the print function to view the table.** 
...
print(top_airlines)
```
**Answer: United, JetBlue, ExpressJet, Delta, American, Envoy, US Airways, Endeavor, Southwest, and Virgin America have the most flights departing NYC airports in 2013.** **Approach: I used the same code from question 1, but i used the merge function in order to join the airlines and flights datasets into 1 dataset so the name of the airline would be available instead of just the carrier.**
